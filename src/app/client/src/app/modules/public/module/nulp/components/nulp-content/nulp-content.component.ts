...
@Component({
    selector: 'app-nulp-content',
    templateUrl: './nulp-content.component.html',
    styleUrls: ['./nulp-content.component.css']
})
export class NulpContentComponent implements OnInit, OnDestroy {
    inviewLogs: any = [];
    /**
       * telemetryImpression
      */
    ...
    */
    navigateToPage(page: number): undefined | void {
        ...
        this.route.navigate([this.nulpRoutingUrl, this.pageNumber], {
    ...
    ngOnInit() {
        if (_.includes(this.route.url, '/nulp')) {
            const url = this.route.url.split('/');
            if (url.indexOf('nulp') === 2) {
                this.nulpRoutingUrl = url[1] + '/' + url[2];
            } else {
                this.nulpRoutingUrl = url[1];
            }
        }
        this.filters = {};
        this.dataDrivenFilter = {};
        this.filterType = this.config.appConfig.nulp.filterType;
        this.redirectUrl = this.config.appConfig.nulp.searchPageredirectUrl;
