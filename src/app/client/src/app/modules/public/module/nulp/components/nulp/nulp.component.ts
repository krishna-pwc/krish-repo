...
@Component({
  selector: 'app-nulp',
  templateUrl: './nulp.component.html',
  styleUrls: ['./nulp.component.css']
})
export class NulpComponent implements OnInit, OnDestroy {
  /**
  ...
  */
  ...
  populatePageData() {
    ...
    const filters = _.pickBy(this.filters, value => value.length > 0);
        ...
        filters.mimeType = ["video/x-youtube"];
          ...
          if (apiResponse && apiResponse.sections) {
            ...
            _.forEach(this.caraouselData, (value, index) => {
              if (this.caraouselData[index].contents && this.caraouselData[index].contents.length > 0) {
                const constantData = this.config.appConfig.NulpPage.constantData;
                const metaData = this.config.appConfig.NulpPage.metaData;
                const dynamicFields = this.config.appConfig.NulpPage.dynamicFields;
  ...
  ngOnInit() {
    ...
    this.redirectUrl = this.config.appConfig.nulp.inPageredirectUrl;
    this.getChannelId();
